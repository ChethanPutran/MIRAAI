...
from vision.estimator.models.action_identifier.model import ActionIdentifier
from vision.estimator.object_identifier import ObjectIdentifier
from vision.estimator.depth_estimator import DepthEstimator
from vision.camera.parameters import FOV_H,FOV_V,T_0_cam
from vision.estimator.object_pose import PoseEstimator
from vision.estimator.hand_pose import HandPose
...
class Extractor:
    ...
    def transfer_hand_pose_to_robot(self,handpose):
        return []
    def fit_hand_pose_robot_pose(self,T_handpose_ee):
        return []
    def extract(self, recordings):
        ...
        while True:
            ...
            T_cam_obj = self.object_pose_identifier.get_pose(object_l,object_r)
            ...
            T_0_obj = T_0_cam @ T_cam_obj
            
            
            # 4. Save task object position
            task.set_object_pos(T_0_obj)
            ...
            T_hand = self.hand_pose_estimator.get_pose(frame_left,frame_right)

            T_robot_ee = self.transfer_hand_pose_to_robot(T_hand)

            # 2. Scale hand pose to the meet the hand requirements
            T_robot_ee_scaled = self.fit_hand_pose_robot_pose(T_robot_ee)
            task.set_ee_pos(T_robot_ee_scaled)
