...
if __name__ == "__main__":
    ...
    Use a stereo camera to capture two images simultaneously: one from the left camera and one from the right camera.
    Ensure the images are rectified (aligned) so that corresponding points lie on the same horizontal line (epipolar line)."
    """

    """
    Login to the Raspberry Pi and start the camera server
    """

    from camera.camera_server import start_server
    ...
    # camera_client.recieve_live_capture()

    # # Get the live images
    # img_let,img_right = camera_client.capture_image()

    # ## Load offline stereo images
    # # left_image = cv2.imread('left_image.png', cv2.IMREAD_GRAYSCALE)
    # # right_image = cv2.imread('right_image.png', cv2.IMREAD_GRAYSCALE)

    # de = DepthEstimator()
    # de.get_3Dpoint_depth("images/left.jpg","images/right.jpg",(10,10))

    """ 2. Rectify the Images
    Rectification aligns the two images so that corresponding points are on the same horizontal line. This simplifies the disparity calculation.
    Use OpenCV's cv2.stereoRectify and cv2.initUndistortRectifyMap functions to compute rectification maps and then apply them to the images."
    "   """